...
static void ntp_get_time_from_address_thread(GTask *task, gpointer source_object, gpointer task_data, GCancellable *cancellable);

void ntp_get_time_from_address_aysnc(GInetAddress *address, GAsyncReadyCallback callback, gpointer user_data)
{
...
time_t ntp_get_time_from_address_finish(GAsyncResult *result, GError **error)
{
    ...
    g_return_val_if_fail(g_async_result_is_tagged(result, ntp_get_time_from_address_aysnc), 0);
...
static void ntp_get_time_from_address_thread(GTask *task, gpointer source_object, gpointer task_data, GCancellable *cancellable)
{
    ...
    g_autoptr(GSocket) socket = g_socket_new(g_inet_address_get_family(address), G_SOCKET_TYPE_DATAGRAM, G_SOCKET_PROTOCOL_UDP, &error);
    if (!socket)
    {
    ...
    g_autoptr(GSocketAddress) socket_address = g_inet_socket_address_new(address, NTP_PORT);
    ...
    g_socket_send(socket, (const gchar *)&packet, sizeof(packet), cancellable, &error);
    if (error)
    {
        g_task_return_error(task, error);
        return;
    ...
    g_socket_receive(socket, (gchar *)&packet, sizeof(packet), cancellable, &error);
    if (error)
    {
        g_task_return_error(task, error);
        return;
